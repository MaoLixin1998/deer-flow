...
async def start_run(
    ...
    """
    # 这里相当于 Java 分层里的 Service：
    # Controller(thread_runs.py/runs.py) 只收 HTTP 请求；
    # start_run 负责建 RunRecord、校验模型、写 thread 元数据、组装 LangGraph config，
    # 最后 create_task(run_agent(...)) 把真正的 agent 执行放到后台跑。
    ...
    try:
        ...
        # 后台任务入口。run_agent 会构建 LangGraph agent，调用模型/工具，
        # 并把中间结果发布到 bridge；HTTP 层的 sse_consumer 再把 bridge 事件
        # 转成浏览器能消费的 text/event-stream。
