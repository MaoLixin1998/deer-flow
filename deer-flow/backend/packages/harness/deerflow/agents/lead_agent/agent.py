...
def make_lead_agent(config: RunnableConfig):
    ...
    # 后端 run_agent 通过这个工厂真正创建“主智能体”。
    # config/context 里带来的 model_name、mode、agent_name、is_bootstrap
    # 都会在 _make_lead_agent 里决定：用哪个模型、加载哪个自定义 SOUL.md、
    # 开不开 plan/subagent，以及是否进入创建智能体的 bootstrap 模式。
