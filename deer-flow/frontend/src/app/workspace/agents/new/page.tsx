...
export default function NewAgentPage() {
  ...
  const { thread, sendMessage } = useThreadStream({
    // 创建自定义智能体复用了聊天运行链路，但会额外传 is_bootstrap=true。
    // 后端 make_lead_agent 看到 is_bootstrap 后，只开放 setup_agent 等创建所需工具。
    threadId: undefined,
    context: {
      mode: "flash",
      is_bootstrap: true,
  ...
  const handleConfirmName = useCallback(async () => {
    ...
    try {
      // 第一步只检查名字是否可用：GET /api/agents/check?name=...
      // 如果 config.yaml 里 agents_api.enabled=false，后端会返回 403，
      // 前端就显示“服务器未开启自定义智能体管理功能”。
    ...
    setStep("chat");
    // 名字通过后，进入 bootstrap 对话：这不是直接 POST /api/agents，
    // 而是先启动一次普通 agent run，让模型和用户讨论 SOUL.md。
    await sendMessage(
      threadId,
      {
  ...
  const handleSaveAgent = useCallback(async () => {
    ...
    try {
      // 保存按钮发送一条隐藏消息，要求 bootstrap agent 调用 setup_agent 工具。
      // setup_agent 工具最终在后端文件系统写入 agents/<name>/config.yaml 和 SOUL.md。
      await sendMessage(
        threadId,
