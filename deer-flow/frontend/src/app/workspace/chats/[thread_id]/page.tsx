...
export default function ChatPage() {
  ...
  const {
    ...
    // 普通聊天页的核心链路入口：
    // InputBox -> handleSubmit -> sendMessage -> LangGraph SDK
    // -> POST /api/threads/{threadId}/runs/stream -> 后端 start_run/run_agent。
  ...
  const handleSubmit = useCallback(
    (message: PromptInputMessage) => {
      // InputBox 只负责收集用户输入；真正的上传、构造 human message、
      // 发起 SSE 流式运行都在 useThreadStream.sendMessage 里完成。
