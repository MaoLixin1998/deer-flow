...
export function useThreadStream({
  ...
  // 前端聊天链路的“Service 层”：
  // 1. 管理历史消息、乐观消息、SSE 流式状态；
  // 2. 把用户输入封装成 LangGraph input/context；
  // 3. 调用 thread.submit，由 LangGraph SDK 访问后端 runs/stream 接口。
  const { t } = useI18n();
  ...
  const sendMessage = useCallback(
    async (
      ...
      try {
        ...
        // 这是前端真正发起聊天运行的位置。
        // SDK 会把第一个参数作为 graph input，把第二个参数转成运行配置：
        // - threadId: 会映射到 URL /api/threads/{threadId}/runs/stream
        // - context: DeerFlow 自定义运行上下文，如模型、模式、自定义智能体名
        // - streamSubgraphs/streamResumable: 控制后端 SSE 返回哪些事件
        await thread.submit(
          {
            messages: [
              {
