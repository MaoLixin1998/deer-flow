...
```text
如果项目已经明确采用“SVG 先行，再同步 Figma”的设计流程，
并且 Figma 因页面数量、账号计划、调用额度或工具不可用暂时受限，
可以把用户确认过的 SVG 高保真方案登记为设计基线。
登记位置必须是 phases/phase-xx/README.md 和独立设计验收文档。
该设计基线必须列明页面状态、交互规则、图标规则、字段映射和后续 Figma 补齐项。
满足以上条件后，可以进入高保真代码原型。
但 Figma 同步不能被删除，只能作为后续补齐项。
```
