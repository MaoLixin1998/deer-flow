...
11. 代码不能只做 Demo，要能讲工程化。
12. 用户没有明确冻结需求前，不能擅自替用户拍板进入编码。
```
...
11. 前端要做真实可用的高保真工作台，不做营销首页，不做纯聊天 Demo。
12. 用户没有明确冻结需求前，不能擅自替用户拍板进入编码。
13. 修改文件前先说明你要改什么；修改后必须做检查并汇报。
...
6. 前端先写死字段，后端再迁就前端。
7. Java/Python 命名不一致。
8. 真实模型接太早，mock 链路没跑通。
9. 只做页面，没有 contracts、后端、测试和验收。
10. 修改了文档但没更新总控。
11. 用户还没确认，就把草案写成最终结论。
```
